  ...
  try {
    ...
    const gpt = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `你是亞鈺汽車客服助手，必須返回有效的 JSON 格式：{"category":"cars"|"other","params":{}, "followup":"..."}。若問車輛數量（如"benz有幾台"），category 設 "cars"，params 含"廠牌"條件（如"benz"或"賓士"），followup 回覆數量。若問具體車輛（如"哪5台"），根據前次條件從 cars 表查詢並列出詳情。cars 表欄位：物件編號, 廠牌, 車款, 車型, 年式, 年份, 變速系統, 車門數, 驅動方式, 引擎燃料, 乘客數, 排氣量, 顏色, 安全性配備, 舒適性配備, 首次領牌時間, 行駛里程, 車身號碼, 引擎號碼, 外匯車資料, 車輛售價, 車輛賣點, 車輛副標題, 賣家保證, 特色說明, 影片看車, 物件圖片, 聯絡人, 行動電話, 賞車地址, line, 檢測機構, 查定編號, 認證書。回覆簡潔，列出最多5台車（廠牌、車款、售價），並將"benz"視為"賓士"。`
        },
        ...trimmedContext,
    ...
    try {
      ...
      console.error("GPT JSON 解析錯誤:", e.message, "Raw output:", gpt.choices[0].message.content);
      await replyToLine(replyToken, "不好意思，系統解析錯誤，請稍後再試或聯繫我們！");
      return res.status(200).json({ status: "ok", message: `GPT JSON 解析錯誤: ${e.message}` });
    }
    ...
    if (category === "cars" && Object.keys(params).length > 0) {
      const query = Object.entries(params)
        .filter(([key]) => ["廠牌", "車輛售價"].includes(key))
        .filter(([_, value]) => value !== undefined && value !== null)
        .map(([key, value]) => {
          if (key === "廠牌" && (value === "benz" || value === "賓士")) return "廠牌=ilike.%賓士%";
    ...
    memory[userId].push(`AI: ${replyText}`);
