...
const roleInstructionsPinecone = `你是亞鈺汽車50年資深客服專員，請根據參考資料中提供的制度說明（如保固、五日鑑賞、估價流程等）分析客人問題類型並判斷是否符合條件。禁止虛構內容，只能依據參考資料回答，可做條件判定與友善引導，若找不到答案，請誠懇告知客戶查無相關資料。`;

const roleInstructionsSupabase = `你是亞鈺汽車的客服助手，請根據車輛資料進行快速摘要，包括廠牌、年份、型號、配備亮點、里程數與是否外匯等資訊，用自然口語方式回覆客人詢問車輛內容。不得自行推論未記載於資料庫中的配備或條件內容。`;
...
export async function querySmartReply(userInput, userId) {
  try {
    ...
    const messages = [
      ...
      {
        role: 'user',
        content: `參考資料如下：\n${context}\n\n請根據以上參考資料內容，分析客戶問題並給出回應。禁止根據參考資料以外的資訊進行回答。問題：${userInput}`
      }
    ];
...
async function querySupabaseContext(query) {
  try {
    ...
    const fieldsToShow = ['廠牌', '年份', '車款', '車型', '行駛里程', '特色說明', '外匯車資料'];
    return fieldsToShow.map(f => match[f] ? `${f}：${match[f]}` : null).filter(Boolean).join('\n');
  } catch (err) {
