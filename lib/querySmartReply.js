...
async function querySupabase(query) {
  try {
    ...
    const jsonbFields = ['安全性配備', '舒適性配備'];
    const textFields = searchFields.filter(f => !jsonbFields.includes(f));

    const orClauses = [
      ...textFields.map(field => `${field}.ilike.${encoded}`),
      ...jsonbFields.map(field => `${field}::text.ilike.${encoded}`),
    ];
    ...
    return `【${data.廠牌 || ''} ${data.車款 || ''}】${data.車輛副標題 || ''}，${data.賣家保證 || ''}，配備亮點：${data.特色說明 || data.車輛賣點 || '尚未提供'}`;
  } catch (err) {
